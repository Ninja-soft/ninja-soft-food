...
// Selector de rango con presets — port del POS (calendario .rdp-ninja en verde).
    ...
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        <button
          type="button"
          className={cn(
            "inline-flex h-11 items-center gap-2 rounded-lg border border-input bg-background px-4 text-sm text-foreground outline-none transition hover:border-primary/50 focus:border-primary focus:ring-4 focus:ring-primary/15",
            className,
          )}
        >
      ...
      <Popover.Portal>
        <Popover.Content
          ...
          sideOffset={6}
          collisionPadding={12}
          className="z-50 max-h-[min(38rem,calc(100dvh-1.5rem))] w-[calc(100vw-1.5rem)] overflow-auto rounded-lg border border-border bg-popover p-3 text-popover-foreground shadow-soft outline-none sm:w-auto"
        >
