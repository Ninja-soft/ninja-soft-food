...
- [~] **Fase 8 — Ecosistema** (parcial): HECHO autónomo — webhooks salientes emitiendo (outbox 0023 + cron 5min, eventos production/dispatch/stock, API v1 completa) y **multi-establecimiento completo** (diseño docs/12, migración 0024 + hotfix 0025 — lección: cambiar firma de RPC exige DROP de la vieja en el mismo archivo o PostgREST da PGRST203 ambiguo —, selector de planta en AppShell, filtros estrictos por planta, gestión con límite de plan). PENDIENTE (requiere a Lucas): delivery (cuentas developer ML/PedidosYa/Rappi/Uber), SSO POS↔Food, conector Ninja POS, Stripe/PayPal (entidad), `@ninja-soft/ui`.
