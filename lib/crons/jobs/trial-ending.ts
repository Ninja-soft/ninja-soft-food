...
// =============================================================================
// lib/crons/jobs/trial-ending — núcleo del aviso diario de prueba por vencer.
//
...
//
// Best-effort por tenant. Sin autorización ni respuesta HTTP: lo invocan el
// route handler (/api/cron/trial-ending, disparo manual) y /api/cron/daily.
// =============================================================================
...
export async function runTrialEnding(): Promise<Summary> {
  ...
  if (subsErr) {
    return { processed: 0, errors: [`subs_query: ${subsErr.message}`] };
  }
