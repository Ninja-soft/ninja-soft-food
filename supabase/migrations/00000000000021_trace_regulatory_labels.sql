-- ============================================================
-- Ninja Food — 0021 trace regulatory_labels en el snapshot inmutable
-- ------------------------------------------------------------
-- La RPC complete_production (0005) snapshotea solo `front_labels` (octógonos
-- Ley 27.642, AR-only, DEPRECATED desde 0013). Un tenant NO argentino produce
-- y su traza pública queda SIN sellos: el rotulado por país vive en
-- recipes.regulatory_labels ({"system":"...","values":[...]}, migración 0013) y
-- la RPC nunca lo copiaba.
--
-- Fix: CREATE OR REPLACE de complete_production con el MISMO cuerpo de 0005
-- (0016 solo agregó la columna productions.photo_url, NO tocó esta función) y un
-- único agregado al payload de la traza: 'regulatory_labels'. v_recipe ya trae
-- la columna porque la RPC hace `select * into v_recipe`.
--
-- INMUTABILIDAD (regla dura 5): este cambio SOLO afecta a producciones NUEVAS.
-- Las public_traces ya escritas NO se tocan — la traza pública
-- (app/(public)/t/[slug]/page.tsx) ya lee payload.regulatory_labels con fallback
-- a payload.front_labels (octógonos AR legacy), así que las trazas viejas siguen
-- mostrando sus sellos sin reescritura.
-- SECURITY INVOKER (default): RLS aplica con el JWT del usuario, igual que 0005.
-- ============================================================
